...
class Pokemon
  ...
  def level=(value)
    if value < 1 #|| value > GameData::GrowthRate.max_level #by low (for master mode)
  ...
  def learn_move(move_id)
    ...
    @moves.shift if numMoves > MAX_MOVES && self.isBossPokemon? #by low
  end
  ...
  def check_evolution_internal
    ...
    return nil if isSpecies?(:MAGIKARP) && true #by low
  ...
  def calc_stats
    ...
    GameData::Stat.each_main do |s|
      if s.id == :HP
        ...
        stats[s.id] = calcStat(base_stats[s.id], this_level, this_IV[s.id], @ev[s.id], nature_mod[s.id], s.id)
      end
    end
