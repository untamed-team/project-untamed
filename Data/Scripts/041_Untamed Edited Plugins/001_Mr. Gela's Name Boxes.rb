...
#===================GELA FACE1=======================================                             
  ...
  def initialize(face)
    ...
    #edited by Gardenette for Full Portraits
    #super(0,0,192,192)
    super(0,0,256,300)
    
    self.windowskin=nil
    faceinfo=face.split(",")
    facefile=pbResolveBitmap("Graphics/Pictures/Faces/"+faceinfo[0])
    facefile=pbResolveBitmap("Graphics/Pictures/"+faceinfo[0]) if !facefile
    self.contents&.dispose
    @faceIndex=faceinfo[1].to_i
  ...
  #  @facebitmaptmp=AnimatedPortraitBitmap.new(facefile)
    ...
    @facebitmap=BitmapWrapper.new(256,300)
    @facebitmap.blt(0,0,@facebitmaptmp.bitmap,Rect.new(
       (@faceIndex % 4) * 300,
       (@faceIndex / 4) * 300, 300, 300
    ))
    self.contents=@facebitmap
  end
...
#===================GELA FACE 6 ===============================		
  ...
  # YOU MAY ADD TEXT SKIP IN THIS PLACE . NOT PROPER TO DISTRIBUTE ANOTHER RESOURCE
  # TOGETHER WITH THIS ONE WITHOUT CREDIT
