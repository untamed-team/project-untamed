...
class Battle::Scene::PokemonDataBox < Sprite
  ...
  def initializeOtherGraphics(viewport)
    ...
    @hpNumbers = BitmapSprite.new(124, 18, viewport)
  ...
  def draw_name
    ...
    nameOffset = nameWidth - 124 if nameWidth > 124
    pbDrawTextPositions(self.bitmap,
      #[[@battler.name, @spriteBaseX + 8 - nameOffset, 12, false, NAME_BASE_COLOR, NAME_SHADOW_COLOR]]
      [[@battler.name, @spriteBaseX + 8 - nameOffset, 10, false, NAME_BASE_COLOR, NAME_SHADOW_COLOR]]
    )
  end

  def draw_level
		if !@battler.isBossPokemon? #by low
			# "Lv" graphic
			pbDrawImagePositions(self.bitmap,
				#[["Graphics/Pictures/Battle/overlay_lv", @spriteBaseX + 140, 16]]
				[["Graphics/Pictures/Battle/overlay_lv", @spriteBaseX + 152, 14]]
			)
			# Level number
			#pbDrawNumber(@battler.level, self.bitmap, @spriteBaseX + 162, 16)
			pbDrawNumber(@battler.level, self.bitmap, @spriteBaseX + 178, 14)
		end
  end
  ...
  def draw_gender
    ...
    pbDrawTextPositions(self.bitmap, [[gender_text, @spriteBaseX + 136, 10, false, base_color, shadow_color]]) if !@battler.isBossPokemon? #by low
  end

  def draw_status
    return if @battler.status == :NONE
    if @battler.status == :POISON && @battler.statusCount > 0 && $game_variables[MECHANICSVAR] < 3 #by low  # Badly poisoned
