class Battle::Move
  ...
  def physicalMove?(thisType = nil)
    return (@category == 0) if Settings::MOVE_CATEGORY_PER_MOVE && !$game_switches[OLDSCHOOLBATTLE]
    thisType ||= @calcType
    thisType ||= @type
  ...
  def specialMove?(thisType = nil)
    return (@category == 1) if Settings::MOVE_CATEGORY_PER_MOVE && !$game_switches[OLDSCHOOLBATTLE]
    thisType ||= @calcType
    thisType ||= @type
  ...
  def bladeMove?;         return @flags.any? { |f| f[/^Blade$/i] };               end #by low
  def doublePPMove?;      return @flags.any? { |f| f[/^DoublePPUse$/i] };         end #by low
  def boostedByGravity?;  return @flags.any? { |f| f[/^GravityBoosted$/i] };      end #by low
  ...
  def usableWhenTruanting? # Truant buff #by low
    return true if healingMove?
    return false
  end
