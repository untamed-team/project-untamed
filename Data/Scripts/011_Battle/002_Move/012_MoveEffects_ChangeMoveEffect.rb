  ...
  def pbEffectAgainstTarget(user, target)
    ...
    target.allAllies.each do |b|
      ...
      target.effects[PBEffects::Spotlight] = b.effects[PBEffects::Spotlight] + 1
		...
		# Showtime (form change) #by low
		if target.isSpecies?(:STRELAVISON) && target.ability == :SHOWTIME &&
			 !@battle.battleBond[target.index & 1][target.pokemonIndex]
			@battle.battleBond[target.index & 1][target.pokemonIndex] = true
			...
			target.pbChangeForm(1, _INTL("Now is my time to shine!"))
...
#===============================================================================
# Damages target if target is a foe, or heals target by 1/3 of its max HP if
...
class Battle::Move::HealAllyOrDamageFoe < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if !target.canHeal? || (target.hp >= target.totalhp / 2)
      @battle.pbDisplay(_INTL("But it failed!")) if show_message
      return true
    end
    return false
...
class Battle::Move::CurseTargetOrLowerUserSpd1RaiseUserAtkDef1 < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    user.pbReduceHP(user.totalhp / 8, false, false) # 2 -> 8 #by low
...
class Battle::Move::ReplaceMoveWithTargetLastMoveUsed < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
		moves = []
		user.pokemon.getMoveList.each do |m|
			moves.push(m[1])
		end
    user.eachMoveWithIndex do |m, i|
      next if m.id != @id
      newMove = Pokemon::Move.new(target.lastRegularMoveUsed)
			...
			if !user.isSpecies?(:SMEARGLE) && 
				 !user.pokemon.compatible_with_move?(newMove.id) && !moves.include?(newMove.id)
				user.pokemon.sketchMove = newMove.id
				#~ print user.pokemon.sketchMove
			end
