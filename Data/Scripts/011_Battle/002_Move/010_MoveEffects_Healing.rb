...
# half of the target's Attack stat (after applying stat stages, before this move 
# decreases it). (Strength Sap) edited #by low
#===============================================================================
...
class Battle::Move::UserLosesHalfOfTotalHPExplosive < Battle::Move
  ...
  def pbSelfKO(user)
    return if !user.takesIndirectDamage?
		if user.hasActiveAbility?(:PARTYPOPPER) #by low
      @battle.pbShowAbilitySplash(user)
			user.pbOwnSide.effects[PBEffects::PartyPopper] = true
...
class Battle::Move::StartPerishCountsForAllBattlers < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    target.effects[PBEffects::PerishSong]     = 99
  ...
  def pbMoveFailed?(user, targets)
    if Settings::MECHANICS_GENERATION >= 7 && user.effects[PBEffects::DestinyBondPrevious]
      ...
      return true
		...
		if user.hp<user.totalhp #by low
      @battle.pbDisplay(_INTL("But it failed!"))
      return true
    end
    return false
