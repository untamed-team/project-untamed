...
class Battle::Move::BadPoisonTarget < Battle::Move::PoisonTarget
  ...
  def pbOverrideSuccessCheckPerHit(user, target)
    return ((Settings::MORE_TYPE_EFFECTS && !$game_switches[OLDSCHOOLBATTLE]) && statusMove? && user.pbHasType?(:POISON))
  end
end
...
class Battle::Move::GiveUserStatusToTarget < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    when :DIZZY
      target.pbDizzy
      msg = _INTL("{1} regained focus.", user.pbThis)
    end
    if msg != ""
...
class Battle::Move::CureUserPartyStatus < Battle::Move
  ...
  def pbAromatherapyHeal(pkmn, battler = nil)
    ...
    when :DIZZY
      @battle.pbDisplay(_INTL("{1} regained focus.", curedName))
    end
  end
...
# (Fake Out)
#===============================================================================
class Battle::Move::FlinchTargetFailsIfNotUserFirstTurn < Battle::Move
  def flinchingMove?; return true; end
