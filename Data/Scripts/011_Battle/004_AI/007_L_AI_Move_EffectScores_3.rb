...
# added the new status nerf if $game_variables[MECHANICSVAR] is above or equal to 3 #by low
  ...
  def pbGetMoveScoreFunctionCode(score, move, user, target, skill = 100)
    ...
    when "FixedDamageHalfTargetHP"
		...
		score *= 1.5 if (target.hp/1.5)<=target.totalhp
		score *= (target.hp * 100 / target.totalhp)/100
    #---------------------------------------------------------------------------
				...
				if target.hasActiveAbility?(:ANGERPOINT)
					if target.attack>target.spatk
						if thisinitial>99
							...
							if pbRoughStat(target,:SPEED,skill) > pbRoughStat(enemy1,:SPEED,skill) && pbRoughStat(target,:SPEED,skill) > pbRoughStat(enemy2,:SPEED,skill)
								score*=1.3
							else
								score*=0.7
							end
							...
							score *= -1
							score = 0 if target.statStageAtMax?(:ATTACK)
						end
					end
				else
					score=0
    ...
    when "TypeAndPowerDependOnWeather" # weather ball
		if ([:Rain, :HeavyRain].include?(user.effectiveWeather) || globalArray.include?("sun weather")) && 
		   target.hasActiveAbility?([:DRYSKIN, :STORMDRAIN, :WATERABSORB],false,mold_broken)
			score*=0.1
		elsif ([:Sun, :HarshSun].include?(user.effectiveWeather) || globalArray.include?("rain weather")) && 
					...
					if target.hasActiveAbility?(:ANGERPOINT)
						if thisinitial>99
							...
							user.eachOpposing do |m|
								next unless pbRoughStat(target,:SPEED,skill) < pbRoughStat(m,:SPEED,skill)
								enemycounter += 1
							end
							if enemycounter == 0
								score*=1.3
    ...
    when "HealUserFullyAndFallAsleep" # rest
		...
		if user.effects[PBEffects::LeechSeed]>=0 && !fastermon && canSleepTarget(target,user,globalArray)
			score *= 0.3 
		end	
    ...
    when "HealUserHalfOfTotalHP", "HealUserHalfOfTotalHPLoseFlyingTypeThisTurn", 
		...
		if move.function == "HealUserDependingOnWeather" 
			...
			halfhealth=(user.totalhp*2/3) if user.hasActiveAbility?(:PRESAGE) || globalArray.include?("sun weather")
			...
			halfhealth=(user.totalhp*2/3) if globalArray.include?("sand weather")
		else     
			halfhealth=(user.totalhp/2)
		end       
    ...
    when "HealUserByHalfOfDamageDone" # drain punch
		...
		if (user.hp!=user.totalhp || 
				((pbRoughStat(user,:SPEED,skill)>pbRoughStat(target,:SPEED,skill)) ^ (@battle.field.effects[PBEffects::TrickRoom]!=0))) && target.effects[PBEffects::Substitute]==0
			score*=miniscore
		end
    #---------------------------------------------------------------------------
    ...
    when "UserConsumeBerryRaiseDefense2"
		if !user.item || !user.item.is_berry? || !user.itemActive?
			...
			score *= 1.2 if user.canHeal? && user.hp < user.totalhp / 3 && user.hasActiveAbility?(:CHEEKPOUCH)
		...
		if target.effects[PBEffects::Substitute]==0 #&& target.item.is_berry?
			case target.item
				...
				when :CUSTAPBERRY, :SALACBERRY
					score*=1.1
					score*=1.4 if ((pbRoughStat(user,:SPEED,skill)<pbRoughStat(target,:SPEED,skill) && @battle.field.effects[PBEffects::TrickRoom]!=0))
				when :NYLOBERRY
					score*=0.8
			end 
			useful_berries = [
		   ...
		   user.effects[PBEffects::Embargo]>0 || @battle.field.effects[PBEffects::MagicRoom]>0
			...
			case user.item_id
				...
				when :KINGSROCK, :RAZORCLAW
					if canFlinchTarget(user,target,mold_broken) && (pbRoughStat(user,:SPEED,skill)>pbRoughStat(target,:SPEED,skill) && @battle.field.effects[PBEffects::TrickRoom]!=0)
						score*=1.3
					end
				when :POWERHERB
					score=0
    ...
    when "RedirectAllMovesToUser" # follow me
		if user.allAllies.length == 0
			...
			target.allAllies.each do |m|
				speedcheck+= 1 if pbRoughStat(user,:SPEED,skill) < pbRoughStat(m, :SPEED, skill)
			end
		...
		if user.allAllies.length == 0
			...
			if user.opposes?(target) # is enemy
				...
				target.allAllies.each do |barget|
					...
					if ((pbRoughStat(user,:SPEED,skill)<pbRoughStat(barget,:SPEED,skill)) ^ (@battle.field.effects[PBEffects::TrickRoom]!=0))
						score*=1.5
					end
				end
				score *= -1
    ...
    when "HealAllyOrDamageFoe" # pollen puff
		if user.opposes?(target) # is enemy
			...
			target.allAllies.each do |barget|
				if target.hp*(1.0/target.totalhp)>0.8
					if ((pbRoughStat(user,:SPEED,skill)<pbRoughStat(target,:SPEED,skill)) ^ (@battle.field.effects[PBEffects::TrickRoom]!=0)) && 
							((pbRoughStat(user,:SPEED,skill)<pbRoughStat(barget,:SPEED,skill)) ^ (@battle.field.effects[PBEffects::TrickRoom]!=0))
						miniscore*=0.5
					else
						miniscore*=0
					end
			...
			user.allAllies.each do |bally|
				target.allAllies.each do |barget|
					if ((pbRoughStat(user,:SPEED,skill)<pbRoughStat(target,:SPEED,skill)) ^ (@battle.field.effects[PBEffects::TrickRoom]!=0)) && 
						...
						if pbRoughStat(bally,:SPEED,skill)<pbRoughStat(target,:SPEED,skill) && 
    ...
    when "UseMoveDependingOnEnvironment" # nature power
		...
		score = pbGetMoveScore(naturemove, user, target, skill, true)
    #---------------------------------------------------------------------------
    ...
    when "SwitchOutUserStatusMove"
		...
		if userlivecount>1
			...
			maxdam = bestmove[0]
			if maxdam <= ((target.hp * 100/target.totalhp) / 4)
    ...
    when "StartSlowerBattlersActFirst" # trick room
		...
		if pbRoughStat(user,:SPEED,skill) < pbRoughStat(target, :SPEED, skill) || user.hasActiveItem?(:IRONBALL)
			if @battle.field.effects[PBEffects::TrickRoom] > 0         
				score=0
			else
				score*=2
    ...
    when "HigherPriorityInGrassyTerrain"
		aspeed = pbRoughStat(user, :SPEED, skill)
		ospeed = pbRoughStat(target, :SPEED, skill)
		speedcheck = aspeed < ospeed
		score *= 1.4 if (@battle.field.terrain == :Grassy || globalArray.include?("grassy terrain")) && user.affectedByTerrain? && speedcheck
    #---------------------------------------------------------------------------
