class Battle::AI
  ...
  def pbGetMoveScoreFunctionCode(score, move, user, target, skill = 100)
    ...
    when "DoublePowerIfUserHasNoItem" # Acrobatics
      if move.pbCalcType(user) == :FLYING && user.hasActiveItem?(:FLYING) && 
         user.hasActiveAbility?(:UNBURDEN)
        score *= 1.3
        score *= 1.3 if !userFasterThanTarget
      end
    #---------------------------------------------------------------------------
    ...
    when "AlwaysCriticalHit" # frost breath
      if target.hasActiveAbility?([:BATTLEARMOR, :SHELLARMOR],false,mold_broken) || user.effects[PBEffects::LaserFocus] > 0
        ...
        if user.opposes?(target) # is enemy
          if (target.hasActiveAbility?(:ANGERPOINT) && target.pbCanRaiseStatBySource?(:ATTACK, :ANGERPOINT)) && 
          ...
          if target.hasActiveAbility?(:ANGERPOINT)
            if !targetSurvivesMove(move,user,target) || target.pbCanRaiseStatBySource?(:ATTACK, :ANGERPOINT)
              score=0
            else
              score = -100.0
              score *= 1.2
    ...
    #---------------------------------------------------------------------------
    when "StartPreventCriticalHitsAgainstUserSide" # Lucky Chant
    ...
    when "StartWeakenElectricMoves" # Mud Sport
      if @battle.field.effects[PBEffects::MudSportField] == 0
        ...
        @battle.pbParty(target.index).each do |pkmn|
          ...
          elecvar += 2 if pkmn.hasType?(:ELECTRIC)
          pkmn.eachMove do |m|
            elecvar += 1 if m.type == :ELECTRIC
          end
        end
        if elecvar > 0
    ...
    when "StartWeakenFireMoves" # Water Sport
      if @battle.field.effects[PBEffects::WaterSportField] == 0
        ...
        @battle.pbParty(target.index).each do |pkmn|
          next if !pkmn || !pkmn.able?
          firevar += 2 if pkmn.hasType?(:FIRE)
          pkmn.eachMove do |m|
            firevar += 1 if m.type == :FIRE
          end
        end
        if firevar > 0
          @battle.pbParty(user.index).each_with_index do |pkmn, idxPkmn|
    ...
    when "RemoveScreens" # brick break
      if user.pbOpposingSide.effects[PBEffects::Reflect] > 0
        score*=1.3
        score*=1.4 if move.physicalMove?(move.type)
      end
      if user.pbOpposingSide.effects[PBEffects::LightScreen] > 0
        score*=1.3
        score*=1.4 if move.specialMove?(move.type)
      end
    ...
    when "TypeAndPowerDependOnTerrain" # Terrain Pulse
      if user.affectedByTerrain?
        if expectedTerrain == :Electric && 
           ...
           hasTypeAI?(:GROUND, target, user, skill))
            score*=0.1
        end
        if expectedTerrain == :Grassy && target.hasActiveAbility?(:SAPSIPPER,false,mold_broken)
          score*=0.1
        end
        if expectedTerrain == :Psychic && hasTypeAI?(:DARK, target, user, skill)
          score*=0.1
        end
      end
    #---------------------------------------------------------------------------
      ...
      if (userFasterThanTarget || priorityAI(user, move, globalArray) > 0) && targetWillMove?(target)
        ...
        if user.hasActiveAbility?(:LIGHTNINGROD)
          if user.spatk > user.attack
            score*=1.5
          elsif user.pbCannotRaiseStatBySource?(:SPECIAL_ATTACK, :LIGHTNINGROD)
            score*=0.1
          end
        end
        if user.allAllies.any?
          user.allAllies.each do |b|
            next unless b.hasActiveAbility?(:LIGHTNINGROD)
            if b.spatk > b.attack
              score*=1.5
            elsif b.pbCannotRaiseStatBySource?(:SPECIAL_ATTACK, LIGHTNINGROD)
              score*=0.7
            end
          end
        end
        if user.hasActiveAbility?(:MOTORDRIVE)
          if user.pbCanRaiseStatBySource?(:SPEED, :MOTORDRIVE)
            score*=1.2
          else
            score*=0.1
          end
    ...
    when "NormalMovesBecomeElectric" # ion deluge / plasma fists
      ...
      if (userFasterThanTarget || priorityAI(user, move, globalArray) > 0) && targetWillMove?(target, "dmg")
        if @battle.choices[target.index][2].type == :NORMAL
           @battle.allSameSideBattlers(user.index).each do |b|
            ...
            if b.hasActiveAbility?(:MOTORDRIVE) && b.pbCannotRaiseStatBySource?(:SPEED, :MOTORDRIVE)
              score*=1.3
            end
      ...
      if move.statusMove? && scorebackup >= score
        score*=0.7
        score*=0.2 if target.allAllies.empty?
      end
    #---------------------------------------------------------------------------
    ...
    when "HitTwoToFiveTimes", "HitTwoToFiveTimesOrThreeForAshGreninja", 
      ...
      if move.pbContactMove?(user)
        ...
        score*=0.2 if user.hasActiveAbility?(:SKILLLINK) && badeffect
      end
      if target.hp==target.totalhp && (target.hasActiveItem?(:FOCUSSASH) || target.hasActiveAbility?(:STURDY,false,mold_broken))
        score*=1.3
      end
      ...
      if move.function == "HitTwoToFiveTimesRaiseUserSpd1LowerUserDef1" # scale shot
        ...
        if user.hasActiveAbility?(:CONTRARY) || user.pbOwnSide.effects[PBEffects::StatDropImmunity]
          ...
          if targetSurvivesMove(move,user,target)
            miniscore*=0.9
            if (!userFasterThanTarget && ospeed<(aspeed*(3.0/2.0)) && @battle.field.effects[PBEffects::TrickRoom] == 0) && 
              !user.hasActiveItem?(:WHITEHERB)
              miniscore*=1.4
            else
          ...
          if (userlivecount == 0 || user.trappedInBattle?) && targetlivecount > 0 
            miniscore*=0.7
          end
        end
        miniscore/=100.0
    ...
    when "TwoTurnAttack" # razor wind
      if !user.hasActiveItem?(:POWERHERB)   
        ...
        score*=0.7 if target.allAllies.any?
        score*=0.1 if pbHasSingleTargetProtectMove?(target, false)
        score*=0.9
      else
        score*=1.2
    ...
    when "TwoTurnAttackRaiseUserSpAtkSpDefSpd2" # Geomancy
      ...
      miniscore/=100.0
      if user.pbCanRaiseStatBySource?(:SPECIAL_ATTACK, move)
        score*=miniscore
      end
      miniscore=100 
      ...
      if user.pbCannotRaiseStatBySource?(:SPECIAL_ATTACK, move) && 
         ...
         user.pbCannotRaiseStatBySource?(:SPEED, move)
        score*=0
      end
    #---------------------------------------------------------------------------
    when "TwoTurnAttackChargeRaiseUserDefense1" # Skull Bash
      ...
      miniscore/=100.0
      miniscore=1 if user.pbCannotRaiseStatBySource?(:DEFENSE, move)
      score*=miniscore
    #---------------------------------------------------------------------------
    ...
    when "TwoTurnAttackInvulnerableUnderground" # dig
      ...
      if user.hasActiveItem?(:POWERHERB)
        ...
        score*=0.5 if target.moves.any? { |j| [:BOUNCE,:FLY,:SKYDROP,:DIVE,:DIG,:PHANTOMFORCE,:SHADOWFORCE].include?(j&.id) }
      end
    ...
    when "HealUserAndAlliesQuarterOfTotalHP", 
         "HealUserAndAlliesQuarterOfTotalHPCureStatus" # Life Dew, Jungle Healing 
      count = 0
      @battle.allSameSideBattlers(user.index).each do |b|
        next if b.hp == b.totalhp || !b.canHeal?
        count += 1
        halfhealth = (b.totalhp / 4)
        halfhealth = (halfhealth*1.5) if b.hasActiveItem?(:COLOGNECASE)
        bestmove=bestMoveVsTarget(target,b,skill) # [maxdam,maxmove,maxprio,physorspec]
        maxdam=bestmove[0] 
        maxmove=bestmove[1]
        maxdam=0 if (target.status == :SLEEP && target.statusCount>1)    
        if !targetSurvivesMove(maxmove,target,b)
          if maxdam>(b.hp+halfhealth)
            score=0
          else
            if maxdam>=halfhealth
              if userFasterThanTarget
        ...
        hpchange=(EndofTurnHPChanges(b,target,false,false,true)) # what % of our hp will change after end of turn effects go through
        opphpchange=(EndofTurnHPChanges(target,b,false,false,true)) # what % of their hp will change after end of turn effects go through
        if opphpchange<1 ## we are going to be taking more chip damage than we are going to heal
          oppchipdamage=((target.totalhp*(1-hpchange)))
        end
        thisdam=maxdam#*1.1
        hplost=(b.totalhp-b.hp)
        hplost+=maxdam if !fasterhealing
        if b.effects[PBEffects::LeechSeed]>=0 && !userFasterThanTarget && canSleepTarget(target,b,globalArray)
          score *= 0.3 
        end  
        if hpchange<1 ## we are going to be taking more chip damage than we are going to heal
          chipdamage=((b.totalhp*(1-hpchange)))
          thisdam+=chipdamage
        elsif hpchange>1 ## we are going to be healing more hp than we take chip damage for  
          healing=((b.totalhp*(hpchange-1)))
          thisdam-=healing if !(thisdam>b.hp)
        elsif hpchange<=0 ## we are going to a huge overstack of end of turn effects. hence we should just not heal.
          score*=0
        end
        if thisdam>hplost
          ...
          if @battle.pbAbleNonActiveCount(b.idxOwnSide) == 0 && hplost<=(halfhealth)
            score*=0.01
          end
          if thisdam<=(halfhealth)
            score*=2
        ...
        if (b.hp.to_f)/b.totalhp<0.5
          ...
          if b.hp*4<b.totalhp
            ...
            if b.hp<b.totalhp*0.13
              if b.burned? || b.frozen?
                score*=2
              end
              if b.takesHailDamage? || b.takesSandstormDamage?
                score*=2
              end  
            end      
          end      
        ...
        if b.paralyzed? || b.effects[PBEffects::Attract]>=0 || b.effects[PBEffects::Confusion]>0
          score*=1.1
        end
        if target.poisoned? || target.burned? || target.frozen? || target.effects[PBEffects::LeechSeed]>=0 || target.effects[PBEffects::Curse]
          score*=1.3
        ...
        score*=0.1 if ((b.hp.to_f)/b.totalhp)>0.8
        score*=0.6 if ((b.hp.to_f)/b.totalhp)>0.6
        score*=2 if ((b.hp.to_f)/b.totalhp)<0.25
        score=0 if b.effects[PBEffects::Wish]>0
        if move.function == "HealUserAndAlliesQuarterOfTotalHPCureStatus" && b.pbHasAnyStatus?
          score*=3
          if (b.hp.to_f)/b.totalhp>0.5
            score*=1.5
          else
    ...
    when "StartLeechSeedTarget" # leech seed
      if target.effects[PBEffects::LeechSeed]<0 && !hasTypeAI?(:GRASS, target, user, skill) && target.effects[PBEffects::Substitute]<=0
    ...
    when "UserTakesTargetItem" # covet
      if !target.hasActiveAbility?(:STICKYHOLD) &&
        ...
        when :CHOICEBAND
          miniscore*=1.1 if user.attack>user.spatk
        when :CHOICESPECS
          miniscore*=1.1 if user.spatk>user.attack
        when :BLACKSLUDGE
    ...
    when "TargetTakesUserItem" # bestow
      if !target.hasActiveAbility?(:STICKYHOLD) && user.item && 
        ...
        when :CHOICEBAND
          score*=1.3 if target.attack<target.spatk
        when :CHOICESPECS
          score*=1.3 if target.spatk<target.attack
        when :BLACKSLUDGE
    ...
    when "StartTargetCannotUseItem" # embargo
      ...
      if target.effects[PBEffects::Embargo]>0  && target.effects[PBEffects::Substitute]>0
        ...
        if target.item
          score*=1.1
          case target.item_id
          when :ELECTRICSEED, :GRASSYSEED, :MISTYSEED, :PSYCHICSEED, 
               :LAXINCENSE, :EXPERTBELT, :MUSCLEBAND, :WISEGLASSES, 
               :LIFEORB, :EVIOLITE, :ASSAULTVEST, :MELEEVEST
            score*=1.2
          when :LEFTOVERS, :BLACKSLUDGE
            score*=1.3
          end
          score*=1.4 if target.hp*2<target.totalhp
        end
    ...
    when "StartNegateHeldItems" # magic room
      if @battle.field.effects[PBEffects::MagicRoom] > 0
        ...
        if user.item
          ...
          score*=1.4 if user.hp*2<user.totalhp
        end
        score*=1.4 if user.hasActiveAbility?(:TRICKSTER)
      end
    #---------------------------------------------------------------------------
    ...
    when "RandomlyDamageOrHealTarget" # Present
      score *= 0.6 if user.pbOwnedByPlayer?
    #---------------------------------------------------------------------------
    when "HealAllyOrDamageFoe" # pollen puff
      if user.opposes?(target) # is enemy
      else           # is ally
        miniscore = -90.75 # heal pulse's score
        ...
        score=miniscore #this is a special circumstance
      end
    #---------------------------------------------------------------------------
    ...
    when "PowerUpAllyMove", "HoldingHandsShamefully" # helping hand, Hold Hands (mazah)
      ...
      if user.allAllies.any?
        ...
        user.allAllies.each do |bally|
          user.allOpposing.each do |barget|
            if (aspeed < pbRoughStat(barget,:SPEED,skill)) ^ (@battle.field.effects[PBEffects::TrickRoom]!=0)
              score*=1.2
              if user.hp*(1.0/user.totalhp) < 0.33
                score*=1.5
              end
              if (pbRoughStat(bally,:SPEED,skill) < pbRoughStat(barget,:SPEED,skill))  ^ (@battle.field.effects[PBEffects::TrickRoom]!=0)
                score*=1.5
              end
            end
          end
    ...
    when "UserAddStockpileRaiseDefSpDef1" # stockpile
      ...
      score*=miniscore
      score=0 if user.hasActiveAbility?(:CONTRARY)
      score=0 if user.pbCannotRaiseStatBySource?(:SPECIAL_DEFENSE, move) && 
    ...
    when "UseRandomMove" # metronome
      ...
      while metronomeMove.length < 10
        ...
        next if userMoves.include?(move_data.id)
    ...
    when "SwitchOutTargetDamagingMove" # dragon tail
      if (target.hasActiveAbility?(:DISGUISE,false,mold_broken) && target.form == 0) || target.effects[PBEffects::Substitute]>0
        # only regular dmg 
        # negative prio debuff is handled elsewhere
      else
        miniscore = 100
        if target.pbOwnSide.effects[PBEffects::StealthRock]
          miniscore*=1.3
    ...
    when "HigherPriorityInGrassyTerrain" # grassy glide
      score *= 1.1 if expectedTerrain == :Grassy && user.affectedByTerrain?
    #---------------------------------------------------------------------------
    when "LowerPPOfTargetLastMoveBy4", "LowerPPOfTargetLastMoveBy3" # Spite, eerie spell
      ...
      if lastmove.nil?
        ...
        count=target.moves.count { |i| i.damagingMove? }
        if lastmove.damagingMove? && count==1
          miniscore*=1.2
        end
