class Battle::AI
  ...
  def pbGetMoveScoreFunctionCode(score, move, user, target, skill = 100)
    ...
    #---------------------------------------------------------------------------
    when "SetUserTypesToUserMoveType" # Conversion 
      # little developer lore to whoever is reading this:
      # initially i was thinking about mega pory-z's ability to be named Conversion-Z/Conversion-3
      # since yknow, Conversion for pory, Conversion2 for pory2 and Conversion-Z/3 for pory-z. it made sense.
      # however, since everything needs to be passed through "democracy", it was put into a vooooooot 
      # (and a slow vote at that) ((took an entire fucking week and nothing was decided on))
      # one of the arguments agaisnt Conversion-Z was because "people might think of con-vert, as in switching religions"
      # faggiest reason to reject it, in all meanings of that word
      if user.canChangeType?
        has_possible_type = false
    ...
    when "HitsTargetInSky", "HitsTargetInSkyGroundsTarget" # sky uppercut, smack down
      ...
      if userFasterThanTarget
        ...
        skyattacks = [:BOUNCE,:FLY,:SKYDROP]
        if target.moves.any? { |j| skyattacks.include?(j&.id) }
          miniscore*=1.1
        end
        if targetWillMove?(target)
          miniscore*=1.5 if skyattacks.include?(@battle.choices[target.index][2].id)
        end
      end
      if move.function == "HitsTargetInSkyGroundsTarget"
        if target.airborneAI(mold_broken)
          miniscore*=2 
