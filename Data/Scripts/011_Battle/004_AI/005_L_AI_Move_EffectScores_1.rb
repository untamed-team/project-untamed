  ...
  def pbGetMoveScoreFunctionCode(score, move, user, target, skill = 100)
    ...
    when "AddToxicSpikesToFoeSide" # toxic spikes
      if user.pbOpposingSide.effects[PBEffects::ToxicSpikes] >= 2 ||
        (user.pbOpposingSide.effects[PBEffects::ToxicSpikes] >= 1 && $player.difficulty_mode?("chaos"))
        score = 0
      else
        roles = pbGetPokemonRole(user, target)
        if roles.include?("Lead")
    ...
    when "RaiseUserAccuracy1", "RaiseUserAccuracy2", "RaiseUserAccuracy3"
      miniscore=100
      miniscore+=10 if move.function == "RaiseUserAccuracy2"
      miniscore+=20 if move.function == "RaiseUserAccuracy3"
      ...
      miniscore*=0.7 if target.allAllies.any?
    ...
    when "RaiseUserEvasion1", "RaiseUserEvasion2", "RaiseUserEvasion2MinimizeUser", "RaiseUserEvasion3"
      ...
      score = 0 if $player.difficulty_mode?("chaos") && move.statusMove?
    #---------------------------------------------------------------------------
          ...
          if user.hasActiveAbility?(:RUNAWAY) || user.hasActiveItem?(:SHEDSHELL)
            ...
            if user.turnCount<2
              ...
              if user.isSpecies?(:MAGCARGO) && user.hasActiveAbility?(:SIMPLE) && 
                ...
                if targetWillMove?(target, "status")
                  targetMove = @battle.choices[target.index][2]
                  if targetMove.function == "DisableTargetStatusMoves" ||
                     targetMove.function == "StealAndUseBeneficialStatusMove"
                    score*=0.01
                  else
                    score*=3.5
                  end
    ...
    when "ResetTargetStatStages" # clear smog
      if target.effects[PBEffects::Substitute]<=0
        ...
        minimini=0
        GameData::Stat.each_battle do |s|
          miniscore += 5*target.stages[s.id] if target.stages[s.id]>0
          minimini -= 5*target.stages[s.id] if target.stages[s.id]<0
        end
        miniscore+=minimini
        miniscore+=100
        miniscore/=100.0
    ...
    when "StartUserSideDoubleSpeed" # Tailwind
      if user.pbOwnSide.effects[PBEffects::Tailwind]>0
        ...
        if roles.include?("Lead")
          ...
          if userFasterThanTarget
            score*=0.9
            score*=0.4 if @battle.pbAbleNonActiveCount(user.idxOwnSide)==0
          end
        end
        if @battle.field.effects[PBEffects::TrickRoom]!=0
          miniscore*=0.1
        ...
        score*=0.5 if target.hasActiveAbility?(:SPEEDBOOST)
      end
    #---------------------------------------------------------------------------
