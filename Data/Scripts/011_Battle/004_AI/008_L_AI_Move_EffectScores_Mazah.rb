    ...
    when "ProtectUser" # Protect, Detect
      if user.effects[PBEffects::ProtectRate] > 1
        ...
        if user.poisoned? || user.burned? || user.frozen?
          score*=0.8
          score*=0.3 if user.effects[PBEffects::Toxic]>0
        end   
  ...
  def pbStatusDamage(move)
    moveScoores = {
      ...
      10 => [:ACUPRESSURE, :CAMOUFLAGE, :CHARM, :CONFIDE, :DEFENSECURL, :DECORATE, :EMBARGO, :FLASH, 
             ...
             :TAILWHIP, :TORMENT, :TEARFULLOOK, :WITHDRAW, :EXCITE, :HOWL, :MEDITATE, :GROWTH, :WORKUP],
  ...
  def getAbilityDisruptScore(move,user,target,skill,stop=true)
    ...
    if target.hasActiveAbility?(:KLUTZ)
      if $player.difficulty_mode?("chaos")
        ...
        if [:TOXICORB, :FLAMEORB, :LAGGINGTAIL, :IRONBALL, :STICKYBARB].include?(target.item_id)
        else
          abilityscore*=0.6
        end
      end
  ...
  def pbGetMidTurnGlobalChanges
    ...
    @battle.allBattlers.each do |j|
      ...
      if megaChecks.key?(megaSpecies) && j.pokemon.willmega && 
        ...
        jspeed = pbRoughStat(j,:SPEED,100,nil,nil,false,false)
