...
#~ =end
    ...
    expGained = expFinal - pkmn.exp
		pbDisplayPaused(_INTL("{1} gained no Exp. Points due to the hard level cap.", pkmn.name)) if expcap && showMessages && expGained <= 0 #by low
    return if expGained <= 0
    # "Exp gained" message
    if showMessages
			if isOutsider && !expcap
    ...
    if pbDisplayConfirm(_INTL("Should {1} forget a move to learn {2}?", pkmnName, moveName))
      loop do
        ...
        if forgetMove >= 0
					...
					pkmn.sketchMove = nil if !pkmn.sketchMove.nil? && pkmn.sketchMove == pkmn.moves[forgetMove].id
