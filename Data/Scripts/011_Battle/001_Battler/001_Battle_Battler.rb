class Battle::Battler
  ...
  def status=(value)
    ...
    self.statusCount = 0 if value != :POISON && value != :SLEEP && value != :DIZZY #by low 
  ...
  def pbSpeed(megaSpeed = false)
    ...
    # Other effects
    speedMult *= 1.5 if pbOwnSide.effects[PBEffects::Tailwind] > 0
  ...
  def pbTypes(withType3 = false, megaEvo = false)
    ret = @types.uniq
  ...
  def pbHasType?(type, usedByAI = false)
    return false if !type
		# WillMega STAB calc and misc #by low
    activeTypes = pbTypes(true, usedByAI)
  ...
  def takesHailDamage?
    ...
    return false if inTwoTurnAttack?("TwoTurnAttackInvulnerableUnderground",
                                     "TwoTurnAttackInvulnerableUnderwater")
    return false if hasActiveAbility?([:OVERCOAT, :ICEBODY, :SNOWCLOAK, :FREEZEOVER, :SNOWWARNING, :SLUSHRUSH])
    return false if hasActiveItem?(:SAFETYGOGGLES)
    return true
  end
  ...
  def affectedByPowder?(showMsg = false)
    return false if fainted?
    if pbHasType?(:GRASS) && (Settings::MORE_TYPE_EFFECTS && !$game_switches[OLDSCHOOLBATTLE])
