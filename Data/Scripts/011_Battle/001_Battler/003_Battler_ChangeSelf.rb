class Battle::Battler
  ...
  def pbFaint(showMessage = true)
    ...
    pbOwnSide.effects[PBEffects::LastRoundFaintedPokemon] = [self.index, self.pokemonIndex] # AI stuff #by low
  ...
  def pbChangeTypes(newType)
		if newType.is_a?(Array) #by low
			@types = newType
    elsif newType.is_a?(Battle::Battler)
  ...
  def pbCheckFormOnWeatherChange(ability_changed = false)
    ...
    # Croacrozen - Freeze Over #by low
    if isSpecies?(:CROACROZEN)
      if hasActiveAbility?(:FREEZEOVER)
        newForm = 0
        newForm = 1 if effectiveWeather == :Hail
        if @form != newForm
          @battle.pbShowAbilitySplash(self, true)
          @battle.pbHideAbilitySplash(self)
          pbChangeForm(newForm, _INTL("{1} transformed!", pbThis))
  ...
  def pbCheckForm(endOfRound = false)
    ...
    # Wishiwashi / Strelavison - Schooling #edits #by low
    if [:WISHIWASHI, :STRELAVISON].include?(self.species) && self.ability == :SCHOOLING
      if @level >= 27 && @hp > @totalhp / 4
        if @form != 1
          @battle.pbShowAbilitySplash(self, true)
          @battle.pbHideAbilitySplash(self)
