class Battle
  ...
  def pbCanRun?(idxBattler)
    ...
    return true if battler.pbHasType?(:GHOST) && (Settings::MORE_TYPE_EFFECTS && !$game_switches[OLDSCHOOLBATTLE])
  ...
  def pbRun(idxBattler, duringBattle = false)
      ...
      if $DEBUG && Input.press?(Input::CTRL)
        if pbDisplayConfirm(_INTL("Treat this battle as a win?"))
          ...
          return 1
				...
				if pbDisplayConfirm(_INTL("Would you like to forfeit?")) # Gave up #by low
    ...
    if !duringBattle
      if battler.pbHasType?(:GHOST) && (Settings::MORE_TYPE_EFFECTS && !$game_switches[OLDSCHOOLBATTLE])
        pbSEPlay("Battle flee")
        pbDisplayPaused(_INTL("You got away safely!"))
        @decision = 3
        return 1
