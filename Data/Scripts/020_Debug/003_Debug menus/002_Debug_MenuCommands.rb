...
MenuHandlers.add(:debug_menu, :give_demo_party, {
  ...
  "effect"      => proc {
    party = []
    species = [:MOLLUCOTTA, :QUETZALIL, :PORSITE, :PACUNA, :WRENNER, :MINIMELO]
    species.each do |id|
...
MenuHandlers.add(:debug_menu, :toggle_pokegear, {
  "name"        => _INTL("Toggle DexNav"),
  "parent"      => :player_menu,
  "description" => _INTL("Toggle possession of the DexNav."),
  "effect"      => proc {
    $player.has_dexnav = !$player.has_dexnav
    pbMessage(_INTL("Gave DexNav.")) if $player.has_dexnav
    pbMessage(_INTL("Lost DexNav.")) if !$player.has_dexnav
  }
})
