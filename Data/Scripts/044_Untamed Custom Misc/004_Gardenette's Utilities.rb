...
=end
  ...
  loop do
    #need to detect the BACK button and break if pressed

    pbMessageCustom(_INTL("\\ts[]<icon=music_note> "))
    ...
    pbMessage(_INTL("\\ts[]a trainer and a victor\\wtnp[#{20*3}] \\ts[]deserving of a crown?'\\wtnp[#{20*4.35}]"))
    pbMessage(_INTL("\\ts[]He said 'Will you\\wtnp[#{20*1.7}] \\ts[]defeat them,\\wtnp[#{20*1.7}]"))
    pbMessage(_INTL("\\ts[]the leaders,\\wtnp[#{20*1.5}] \\ts[]and all the other trainers,\\wtnp[#{20*3}]"))
    pbMessage(_INTL("\\ts[]with that Eevee of yours?\\wtnp[#{20*4.5}]"))
    pbMessage(_INTL("\\ts[]Because one day\\wtnp[#{20*1.7}] \\ts[]I'll leave you,\\wtnp[#{20*1.7}]"))
    pbMessage(_INTL("\\ts[]a champion\\wtnp[#{20*1.5}] \\ts[]who got all of the badges,\\wtnp[#{20*3}]"))
    pbMessage(_INTL("\\ts[]and joined the Elite Four.'\\wtnp[#{20*3.5}]"))
    pbMessage(_INTL("\\ts[]<icon=music_note>\\wtnp[#{20*13.9}]"))

    pbMessage(_INTL("\\ts[]When I was\\wtnp[#{20*1.5}] \\ts[]a young boy,\\wtnp[#{20*1.5}] \\ts[]my father\\wtnp[#{20*1.5}] \\ts[]took me to Pewter City\\wtnp[#{20*3}] \\ts[]to see the gym in town.\\wtnp[#{20*4.6}]"))
    pbMessage(_INTL("\\ts[]He said, 'Son when\\wtnp[#{20*1.7}] \\ts[]you grow up,\\wtnp[#{20*1.7}] \\ts[]would you be\\wtnp[#{20*1.7}] "))
    pbMessage(_INTL("\\ts[]a trainer and a victor\\wtnp[#{20*3}] \\ts[]deserving of a crown?'\\wtnp[#{20*10.9}]"))
    pbMessage(_INTL("\\ts[]<icon=music_note>\\wtnp[#{20*9.4}]"))

    pbMessage(_INTL("\\ts[]Sometimes I get the feeling\\wtnp[#{20*2.5}] \\ts[]he's watching over me\\wtnp[#{20*2.3}]"))
    pbMessage(_INTL("\\ts[]and other times I feel like I'm a fool.\\wtnp[#{20*4.7}]"))
    pbMessage(_INTL("\\ts[]And through it all,\\wtnp[#{20*1.5}] \\ts[]the rise and fall,\\wtnp[#{20*1.5}] \\ts[]the vict'ries and defeats,\\wtnp[#{20*2}]"))
    pbMessage(_INTL("\\ts[]and though you're gone I'll make sure that I rule\\wtnp[#{20*2}] \\ts[]with Flareon,\\wtnp[#{20*1.5}] \\ts[]with Flareon,\\wtnp[#{20*2}]"))

  end
end #def mcr_wttbp

#==================================================================================
# Custom Message - Cannot be Progressed by Input
#==================================================================================
def pbMessageCustom(message, commands = nil, cmdIfCancel = 0, skin = nil, defaultCmd = 0, &block)
  ret = 0
  msgwindow = pbCreateMessageWindow(nil, skin)
  if commands
    ret = pbMessageDisplayCustom(msgwindow, message, true,
                           proc { |msgwindow|
                             next Kernel.pbShowCommands(msgwindow, commands, cmdIfCancel, defaultCmd, &block)
                           }, &block)
  else
    pbMessageDisplayCustom(msgwindow, message, &block)
  end
  pbDisposeMessageWindow(msgwindow)
  Input.update
  return ret
...
def pbMessageDisplayCustom(msgwindow,message,letterbyletter=true,commandProc=nil)
  return if !msgwindow
