...
class Game_Temp
  ...
  attr_accessor :party_speed_boost_number
  attr_accessor :party_berries_eaten_number
  attr_accessor :party_fly_turns_number
  attr_accessor :party_dead_bananas
	#
...
EventHandlers.add(:on_start_battle, :record_party_status,
  proc {
    ...
    $game_temp.party_critical_hits_dealt = []
		# the big funny #by low
    $game_temp.party_speed_boost_number = []
    $game_temp.party_berries_eaten_number = []
    $game_temp.party_fly_turns_number = []
    $game_temp.party_dead_bananas = []
		#
    ...
    $player.party.each_with_index do |pkmn, i|
      ...
      $game_temp.party_direct_damage_taken[i] = 0
			# the big funny #by low
      $game_temp.party_speed_boost_number[i] = 0
      $game_temp.party_berries_eaten_number[i] = 0
      $game_temp.party_fly_turns_number[i] = 0
      $game_temp.party_dead_bananas[i] = 0
			#
    end
  }
)
...
EventHandlers.add(:on_end_battle, :evolve_and_black_out,
  proc { |decision, canLose|
    ...
    $game_temp.party_direct_damage_taken = nil
		# the big funny #by low
    $game_temp.party_speed_boost_number = nil
    $game_temp.party_berries_eaten_number = nil
    $game_temp.party_fly_turns_number = nil
    $game_temp.party_dead_bananas = nil
		#
    ...
    when 2, 5   # Lose, draw
      if !canLose
		...
		when 69 		# Gave up #by low
			pbReturnToTitleSceneMap()
    end
  }
)
...
PICKUP_COMMON_ITEMS = [
  ...
  :OLDGATEAU,     # Levels                     31-40, 41-50, 51-60, 61-70, 71-80, 81-90, 91-100
  :MAXREPEL,      # Levels                            41-50, 51-60, 61-70, 71-80, 81-90, 91-100
