...
#~ GameData::Evolution.register({
  #~ :id            => :Level30HasTypeMove,
  #~ :parameter     => :Type,
  #~ :level_up_proc => proc { |pkmn, parameter|
    #~ next pkmn.level >= 30 && pkmn.moves.any? { |m| m && m.type == parameter }
  #~ }
#~ })
...
GameData::Evolution.register({ # the big funny 3: the prequel
  ...
  :after_battle_proc => proc { |pkmn, party_index, parameter|
		if pkmn.level >= parameter
			next $game_temp.party_fly_turns_number &&
					 $game_temp.party_fly_turns_number[party_index] &&
					 $game_temp.party_fly_turns_number[party_index] >= 5
		end
  }
})
