...
#-------------------------------------------------------------------------------
# All dialogues for fainted follower - added by Gardenette
#-------------------------------------------------------------------------------
EventHandlers.add(:following_pkmn_talk, :sad_generic, proc { |pkmn, random_val|
  if random_val == 7
    FollowingPkmn.animation(FollowingPkmn::ANIMATION_EMOTE_SAD)
    pbMoveRoute($game_player, [PBMoveRoute::Wait, 20])
    messages = [
      _INTL("{1} looks like it doesn't feel so good..."),
      _INTL("{1} is so tired it's barely keeping up."),
      _INTL("{1} is looking at {2} with sad eyes."),
      _INTL("{1} seems too tired to battle."),
      _INTL("{1} seems to want to visit a PokéCenter.")
    ]
    value = rand(messages.length)
    # Special move route to go along with some of the dialogue
    pbMessage(_INTL(messages[value], pkmn.name, $player.name))
